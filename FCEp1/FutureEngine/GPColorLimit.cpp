...
GPColorLimit::GPColorLimit() {

    m_VSPath = "engine/deShader/colorLimit.vsh";
    m_PSPath = "engine/deShader/colorLimit.psh";
    CreatePL();
...
void GPColorLimit::CreatePL() {
    ...
    PSOCreateInfo.PSODesc.Name = "ColorLimit PSO";
  ...
  //  ShaderCI.Macros = { Macros, _countof(Macros) };
    ...
    RefCntAutoPtr<IShader> pVS;
    {
        ...
        ShaderCI.Desc.Name = "ColorLimit VS";
        ShaderCI.FilePath = m_VSPath.c_str();
        dev->CreateShader(ShaderCI, &pVS);
        // Create dynamic uniform buffer that will store our transformation matrix
        // Dynamic buffers can be frequently updated by the CPU
        BufferDesc CBDesc;
        CBDesc.Name = "GP3D Constants";
        CBDesc.Size = sizeof(CLConstants);
    ...
    RefCntAutoPtr<IShader> pPS;
    {
        ...
        ShaderCI.EntryPoint = "main";
        ShaderCI.Desc.Name = "ColorLimit PS";
...
void GPColorLimit::Bind() {


    {
        MapHelper<CLConstants> CBConstants(FutureApp::m_Inst->GetContext(), m_Constants, MAP_WRITE, MAP_FLAG_DISCARD);
        CBConstants[0].mvp = m_World;
        CBConstants[0].limit = float4(0.3, 0, 0, 0);
...
void GPColorLimit::Bind(MeshBuffer* buffer, Texture2D* texture) {


    {
        MapHelper<CLConstants> CBConstants(FutureApp::m_Inst->GetContext(), m_Constants, MAP_WRITE, MAP_FLAG_DISCARD);
        CBConstants[0].mvp = m_World;
        CBConstants[0].limit = float4(0.6, 0, 0, 0);
    }
...
void GPColorLimit::Render(MeshBuffer* buffer) {
