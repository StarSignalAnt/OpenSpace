...
Diligent::float4x4 GLMToDiligentMatrix3D4(const glm::mat4& mat)
{
...
void GPSDepth::CreatePL() {
  ...
  //  ShaderCI.Macros = { Macros, _countof(Macros) };
    ...
    RefCntAutoPtr<IShader> pVS;
    {
        ...
        CBDesc.Size = sizeof(RDepthConstants);
...
void GPSDepth::Bind(glm::mat4 world, glm::mat4 view, glm::mat4 proj, NodeCamera* cam, Mesh3DBuffer* buffer,std::vector<float4x4> bones) {
    // Ensure that the constant buffer is up-to-date for the current frame
    {
        MapHelper<RDepthConstants> CBConstants(FutureApp::m_Inst->GetContext(), m_Constants, MAP_WRITE, MAP_FLAG_DISCARD);
        CBConstants[0].world = GLMToDiligentMatrix3D4(world);
        CBConstants[0].view = GLMToDiligentMatrix3D4(view);
        CBConstants[0].proj = GLMToDiligentMatrix3D4(proj);
        CBConstants[0].camPos = float4(cam->GetPosition().x, cam->GetPosition().y, cam->GetPosition().z, 0);
        CBConstants[0].camInfo = float4(cam->GetFarZ(), 0, 0, 0);
        CBConstants[0].mvp = GLMToDiligentMatrix3D4(proj * view * world);;
        for (int i = 0;i < 100;i++) {

            CBConstants[0].bones[i] = bones[i];
...
void GPSDepth::Render(Mesh3DBuffer* buffer) {
