...
Diligent::float4x4 GLMToDiligentMatrix3D3(glm::mat4 mat)
{
...
void GPS3D::Bind(glm::mat4 world, glm::mat4 view, glm::mat4 proj, NodeCamera* cam, NodeLight* light, bool first_light, Mesh3DBuffer* buffer, Texture2D* texture, Texture2D* normal, RenderTargetCube* shadow,std::vector<float4x4> bones) {
    // Ensure that the constant buffer is up-to-date for the current frame
    {
        ...
        CBConstants[0].world = GLMToDiligentMatrix3D3(world);
        CBConstants[0].view = GLMToDiligentMatrix3D3(view);
        CBConstants[0].proj = GLMToDiligentMatrix3D3(proj);
        CBConstants[0].mvp = GLMToDiligentMatrix3D3(proj * view * world);;
...
void GPS3D::Render(Mesh3DBuffer* buffer) {
