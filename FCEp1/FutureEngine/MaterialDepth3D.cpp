...
class NodeLight;
class NodeCamera;
Diligent::float4x4 GLMToDiligentMatrix12(const glm::mat4& mat)
{
...
MaterialDepth3D::MaterialDepth3D() {

    m_VSPath = "engine/deShader/renderDepth.vsh";
    m_PSPath = "engine/deShader/renderDepth.psh";
	InitPipeline();

}


MaterialDepth3D::MaterialDepth3D(bool nothing) {



}

void MaterialDepth3D::InitPipeline() {
...
void MaterialDepth3D::Bind() {

    // Ensure that the constant buffer is up-to-date for the current frame
    {
        MapHelper<DepthConstants> CBConstants(FutureApp::m_Inst->GetContext(), m_Constants, MAP_WRITE, MAP_FLAG_DISCARD);
        CBConstants[0].world = GLMToDiligentMatrix12(m_RenderMatrices[0]);
        CBConstants[0].view = GLMToDiligentMatrix12(m_RenderMatrices[1]);
        CBConstants[0].proj = GLMToDiligentMatrix12(m_RenderMatrices[2]);
...
RenderMaterial* MaterialDepth3D::Clone() {

    MaterialDepth3D* res = new MaterialDepth3D(false);
