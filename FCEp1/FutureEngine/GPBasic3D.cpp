...
Diligent::float4x4 GLMToDiligentMatrix(const glm::mat4& mat)
{
...
void GPBasic3D::CreatePL() {
  ...
  //  ShaderCI.Macros = { Macros, _countof(Macros) };
    ...
    RefCntAutoPtr<IShader> pVS;
    {
        ...
        CBDesc.Size = sizeof(RBConstants);
...
void GPBasic3D::Bind(glm::mat4 world, glm::mat4 view, glm::mat4 proj, Mesh3DBuffer* buffer, Texture2D* texture) {
    // Ensure that the constant buffer is up-to-date for the current frame
    {
        MapHelper<RBConstants> CBConstants(FutureApp::m_Inst->GetContext(), m_Constants, MAP_WRITE, MAP_FLAG_DISCARD);
        CBConstants[0].world = GLMToDiligentMatrix(world);
        CBConstants[0].view = GLMToDiligentMatrix(view);
        CBConstants[0].proj = GLMToDiligentMatrix(proj);
    }
...
void GPBasic3D::Render(Mesh3DBuffer* buffer) {
