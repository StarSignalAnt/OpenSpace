...
class NodeLight;
class NodeCamera;
Diligent::float4x4 GLMToDiligentMatrix13(const glm::mat4& mat)
{
...
MaterialDepthS3D::MaterialDepthS3D() {

    m_VSPath = "engine/deShader/renderSDepth.vsh";
    m_PSPath = "engine/deShader/renderSDepth.psh";
    InitPipeline();
...
void MaterialDepthS3D::InitPipeline() {
    ...
    PSOCreateInfo.PSODesc.Name = "Render-Depth PSO";
  ...
  //  ShaderCI.Macros = { Macros, _countof(Macros) };
    ...
    RefCntAutoPtr<IShader> pVS;
    {
        ...
        ShaderCI.Desc.Name = "Render-Depth VS";
        ShaderCI.FilePath = m_VSPath.c_str();
        dev->CreateShader(ShaderCI, &pVS);
        // Create dynamic uniform buffer that will store our transformation matrix
        // Dynamic buffers can be frequently updated by the CPU
        BufferDesc CBDesc;
        CBDesc.Name = "Render-Depth Constants";
        CBDesc.Size = sizeof(DepthSConstants);
    ...
    RefCntAutoPtr<IShader> pPS;
    {
        ...
        ShaderCI.EntryPoint = "main";
        ShaderCI.Desc.Name = "Render-Depth PS";
...
void MaterialDepthS3D::Bind() {

    // Ensure that the constant buffer is up-to-date for the current frame
    {
        MapHelper<DepthSConstants> CBConstants(FutureApp::m_Inst->GetContext(), m_Constants, MAP_WRITE, MAP_FLAG_DISCARD);
        CBConstants[0].world = GLMToDiligentMatrix13(m_RenderMatrices[0]);
        CBConstants[0].view = GLMToDiligentMatrix13(m_RenderMatrices[1]);
        CBConstants[0].proj = GLMToDiligentMatrix13(m_RenderMatrices[2]);
        CBConstants[0].camPos = float4(m_RenderCamera->GetPosition().x, m_RenderCamera->GetPosition().y, m_RenderCamera->GetPosition().z, 0);
        CBConstants[0].camInfo = float4(m_RenderCamera->GetFarZ(), 0, 0, 0);
        CBConstants[0].mvp = GLMToDiligentMatrix13(m_RenderMatrices[2] * m_RenderMatrices[1] * m_RenderMatrices[0]);;
        for (int i = 0;i < 100;i++) {
