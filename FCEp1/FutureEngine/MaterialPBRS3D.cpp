...
Diligent::float4x4 GLMToDiligentMatrix14(glm::mat4 mat)
{
...
MaterialPBRS3D::MaterialPBRS3D() {

	m_VSPath = "engine/deShader/renderS3D.vsh";
	m_PSPath = "engine/deShader/renderS3D.psh";
	InitPipeline();

}


MaterialPBRS3D::MaterialPBRS3D(bool nothing) {



}

void MaterialPBRS3D::InitPipeline() {
...
void MaterialPBRS3D::Bind() {

    {
        MapHelper<R3DConstants> CBConstants(FutureApp::m_Inst->GetContext(), m_Constants, MAP_WRITE, MAP_FLAG_DISCARD);
        CBConstants[0].world = GLMToDiligentMatrix14(m_RenderMatrices[0]);
        CBConstants[0].view = GLMToDiligentMatrix14(m_RenderMatrices[1]);
        CBConstants[0].proj = GLMToDiligentMatrix14(m_RenderMatrices[2]);
        CBConstants[0].mvp = GLMToDiligentMatrix14(m_RenderMatrices[2] * m_RenderMatrices[1] * m_RenderMatrices[0]);;
        CBConstants[0].lightPos = float4(m_RenderLight->GetPosition().x, m_RenderLight->GetPosition().y, m_RenderLight->GetPosition().z, 1);
        ...
        CBConstants[0].lightInfo = float4(m_RenderLight->GetRange(), 0, 0, 0);
        CBConstants[0].camPos = float4(m_RenderCamera->GetPosition().x, m_RenderCamera->GetPosition().y, m_RenderCamera->GetPosition().z, 0);
        for (int i = 0;i < 100;i++) {
            CBConstants[0].bones[i] = m_RenderBoneMatrices[i];

        }
    }

    const Uint64 offset = 0;
    IBuffer* pBuffs[] = { m_RenderBuffers[0]->GetVertexBuffer()};

    // Set texture
    m_SRB->GetVariableByName(SHADER_TYPE_PIXEL, "g_Texture")->Set(m_Textures[0]->GetView());
    m_SRB->GetVariableByName(SHADER_TYPE_PIXEL, "g_TextureNorm")->Set(m_Textures[1]->GetView());
    m_SRB->GetVariableByName(SHADER_TYPE_PIXEL, "g_ShadowMap")->Set(m_Shadow->GetSRV());
    // 
...
RenderMaterial* MaterialPBRS3D::Clone() {

    MaterialPBRS3D* res = new MaterialPBRS3D(false);
