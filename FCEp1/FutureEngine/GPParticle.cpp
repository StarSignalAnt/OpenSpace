...
Diligent::float4x4 GLMToDiligentMatrix3D5(const glm::mat4& mat)
{
...
GPParticle::GPParticle() {

    m_VSPath = "engine/deShader/renderParticle.vsh";
    m_PSPath = "engine/deShader/renderParticle.psh";
    CreatePL();

}

void GPParticle::CreatePL() {
    ...
    PSOCreateInfo.PSODesc.Name = "RenderParticle PSO";
   ...
   // RT0Blend.SrcBlendAlpha = BLEND_FACTOR_ZERO;
    ...
    // Source factors
    PSOCreateInfo.GraphicsPipeline.BlendDesc.RenderTargets[0].SrcBlend = BLEND_FACTOR_ONE;
    PSOCreateInfo.GraphicsPipeline.BlendDesc.RenderTargets[0].SrcBlendAlpha = BLEND_FACTOR_ZERO;
 ...
 // 
    ...
    PSOCreateInfo.GraphicsPipeline.BlendDesc.RenderTargets[0].BlendOp = BLEND_OPERATION_ADD;
    //PSOCreateInfo.GraphicsPipeline.BlendDesc.RenderTargets[0].LogicOp = LOG
    ...
    PSOCreateInfo.GraphicsPipeline.BlendDesc.RenderTargets[0].RenderTargetWriteMask =
        COLOR_MASK_RED | COLOR_MASK_GREEN | COLOR_MASK_BLUE;
  ...
  //  ShaderCI.Macros = { Macros, _countof(Macros) };
    ...
    RefCntAutoPtr<IShader> pPS;
    {
        ...
        ShaderCI.EntryPoint = "main";
        ShaderCI.Desc.Name = "RenderParticle-PS";
...
void GPParticle::Bind(glm::mat4 world, glm::mat4 view, glm::mat4 proj, NodeCamera* cam, NodeLight* light, bool first_light, Mesh3DBuffer* buffer, Texture2D* texture, Texture2D* normal, RenderTargetCube* shadow) {
    // Ensure that the constant buffer is up-to-date for the current frame
    {
      //  MapHelper<R3DConstants> CBConstants(FutureApp::m_Inst->GetContext(), m_Constants, MAP_WRITE, MAP_FLAG_DISCARD);
       ...
       // CBConstants[0].g_ViewProj = GLMToDiligentMatrix3D5(proj * view);;
...
void GPParticle::Bind(RefCntAutoPtr<IBuffer> VertexBuffer, RefCntAutoPtr<IBuffer> IndexBuffer, RefCntAutoPtr<IBuffer>                m_InstanceBuffer, Texture2D* tex)
{

    {
        // Map the buffer and write current world-view-projection matrix
        MapHelper<float4x4> CBConstants(FutureApp::m_Inst->GetContext(), m_Constants, MAP_WRITE, MAP_FLAG_DISCARD);
        CBConstants[0] = float4x4::Identity();
        //CBConstants[1] = m_RotationMatrix;

    }

    m_SRB->GetVariableByName(SHADER_TYPE_PIXEL, "g_Texture")->Set(tex->GetView());

    const Uint64 offsets[] = { 0, 0 };
    IBuffer* pBuffs[] = { VertexBuffer, m_InstanceBuffer };
    FutureApp::m_Inst->GetContext()->SetVertexBuffers(0, _countof(pBuffs), pBuffs, offsets, RESOURCE_STATE_TRANSITION_MODE_TRANSITION, SET_VERTEX_BUFFERS_FLAG_RESET);
    FutureApp::m_Inst->GetContext()->SetIndexBuffer(IndexBuffer, 0, RESOURCE_STATE_TRANSITION_MODE_TRANSITION);
...
void GPParticle::Render(Mesh3DBuffer* buffer) {
