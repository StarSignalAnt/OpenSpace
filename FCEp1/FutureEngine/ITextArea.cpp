...
void ITextArea::Render() {
   ...
   // );
    ...
    // Note: GL_SCISSOR_TEST handling is ignored as requested
...
void ITextArea::ProcessTextInput(char c) {
    ...
    // Ensure cursor is visible after adding text
    EnsureCursorVisible();
}
...
void ITextArea::EnsureCursorVisible() {
    ...
    if (m_MaxContentHeight <= viewportHeight) {
        // All content fits in viewport, no scrolling needed
        m_VerticalScrollOffset = 0.0f;
        m_VerticalScroller->SetScrollPosition(0.0f);
    }
    else {
    ...
    if (m_AllowHorizontalScroll) {
        ...
        if (m_MaxContentWidth <= viewportWidth) {
            // All content fits horizontally, no scrolling needed
            m_HorizontalScrollOffset = 0.0f;
            m_HorizontalScroller->SetScrollPosition(0.0f);
        }
        else {
...
void ITextArea::AddText(const std::string& text) {
    if (m_IsReadOnly || text.empty())
        return;
    ...
    // Split the input text by newlines while preserving empty lines
    std::vector<std::string> inputLines;
    size_t start = 0;
    size_t end = text.find('\n');

    while (end != std::string::npos) {
        inputLines.push_back(text.substr(start, end - start));
        start = end + 1;
        end = text.find('\n', start);
    }

    // Add the last segment
    inputLines.push_back(text.substr(start));

    // Handle each line of the input text
    for (size_t lineIdx = 0; lineIdx < inputLines.size(); lineIdx++) {
        const std::string& inputLine = inputLines[lineIdx];

        // If this isn't the first line, we need to insert a new line first
        if (lineIdx > 0) {
            // Get text after cursor on current line
            std::string afterCursor = "";
            if (m_CursorColumn < m_Lines[m_CursorLine].length()) {
                afterCursor = m_Lines[m_CursorLine].substr(m_CursorColumn);
            }

            // Truncate current line at cursor position
            m_Lines[m_CursorLine] = m_Lines[m_CursorLine].substr(0, m_CursorColumn);

            // Insert new line with text that was after cursor
            m_Lines.insert(m_Lines.begin() + m_CursorLine + 1, afterCursor);
        ...
        if (!inputLine.empty()) {
            // Calculate max line width before wrapping
            float maxWidth = m_Size.x - 20.0f - (m_Padding * 2);
            ...
            while (pos <= inputLine.size()) {
                // Identify word boundaries (space or end of string)
                bool isWordEnd = pos == inputLine.size() ||
                    inputLine[pos] == ' ' ||
                    inputLine[pos] == '\t';

                if (isWordEnd && pos > wordStart) {
                    // Extract the current word (including trailing space if present)
                    currentWord = inputLine.substr(wordStart, pos - wordStart);
                    if (pos < inputLine.size() && (inputLine[pos] == ' ' || inputLine[pos] == '\t')) {
                        currentWord += inputLine[pos]; // Include the space/tab
                    }

                    // Get the current line up to the cursor
                    std::string currentLinePrefix = m_Lines[m_CursorLine].substr(0, m_CursorColumn);

                    // Check if adding this word would exceed the width
                    float lineWidth = GetLineWidth(currentLinePrefix + currentWord);

                    if (lineWidth > maxWidth && !currentLinePrefix.empty()) {
                        // Wrap to next line before adding the word
                        ...
                        // Update current line to just have the prefix
                        m_Lines[m_CursorLine] = currentLinePrefix;

                        // Create a new line with whatever was after the cursor
                        m_Lines.insert(m_Lines.begin() + m_CursorLine + 1, afterCursor);
                        ...
                        // If the word itself is too long for an empty line, we'll need to split it
                        if (GetLineWidth(currentWord) > maxWidth) {
                            // Add the word character by character, forcing wrap when needed
                            for (char c : currentWord) {
                                std::string testLine = m_Lines[m_CursorLine].substr(0, m_CursorColumn) + c;
                                if (GetLineWidth(testLine) > maxWidth && m_CursorColumn > 0) {
                                    // Wrap to next line
                                    std::string afterCursor = "";
                                    if (m_CursorColumn < m_Lines[m_CursorLine].length()) {
                                        afterCursor = m_Lines[m_CursorLine].substr(m_CursorColumn);
                                ...
                                if (m_CursorColumn >= m_Lines[m_CursorLine].length()) {
                                    m_Lines[m_CursorLine] += c;
                                }
                                else {
                                    m_Lines[m_CursorLine].insert(m_CursorColumn, 1, c);
                                }
                                m_CursorColumn++;
                            }
                        }
                            ...
                            if (m_CursorColumn >= m_Lines[m_CursorLine].length()) {
                                m_Lines[m_CursorLine] += currentWord;
                            }
                            else {
                                m_Lines[m_CursorLine].insert(m_CursorColumn, currentWord);
                            }
                            m_CursorColumn += currentWord.length();
                        }
                    }
                    else {
                        // Add the word at current cursor position
                        if (m_CursorColumn >= m_Lines[m_CursorLine].length()) {
                            m_Lines[m_CursorLine] += currentWord;
                        }
                        else {
                    ...
                    // Update word start for next word
                    wordStart = pos + 1;
                }
