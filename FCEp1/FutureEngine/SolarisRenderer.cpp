	...
	if (dynamic_cast<NodeEntity*>(node)) {
		...
		for (auto mesh : entity->GetMeshes()) {
			...
			for (auto light : lights) {
				m_Tex = mesh->GetMaterial()->GetColor();
				m_GP3D->Bind(entity->GetWorldMatrix(), m_RenderCamera->GetWorldMatrix(), m_RenderCamera->GetProjectionMatrix(),m_RenderCamera,light,true, mesh, m_Tex, mesh->GetMaterial()->GetNormal());
				m_GP3D->Render(mesh);
				int a = 5;
			}
